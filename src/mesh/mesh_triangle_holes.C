...
namespace
{
  ...
  Real simple_det_calculator(const Real a11,
                             const Real a12,
                             const Real a21,
                             const Real a22)
  {
                             ...
                             const Point &edge_pt2)
  {
    ...
    if (simple_det_calculator(edge_pt1(0) - edge_pt0(0),
...
namespace libMesh
{
                                                      ...
                                                      Point ray_target) const
  {
    ...
    for (auto i : make_range(np))
    {
      ...
      if (intersection_distance >= 0)
        intersection_distances.push_back(intersection_distance);
    }
  ...
  Point TriangulatorInterface::Hole::calculate_inside_point() const
  {
    ...
    libmesh_error_msg_if(!intersection_distances.size(),
  ...
  TriangulatorInterface::PolygonHole::PolygonHole(const Point &center,
                                                  Real radius,
                                                  unsigned int n_points_in) : _center(center),
                                                                              _radius(radius),
      ...
      : _center(std::numeric_limits<Real>::max())
  {
    ...
    {
      // Start with any edge
      std::pair<std::vector<const Node *>, int> hole_points_and_edge_type{{hole_edge_map.begin()->first, hole_edge_map.begin()->second.first},
    ...
    auto print_areas = [areas]()
    {
      ...
      for (auto area : areas)
        libMesh::out << '(' << edgenames[area.second] << ' ' << area.first << ')';
